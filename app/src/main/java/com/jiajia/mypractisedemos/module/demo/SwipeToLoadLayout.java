...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    static final String TAG = SwipeToLoadLayout.class.getSimpleName();
     ...
     */
    int mStatus = STATUS.STATUS_DEFAULT;
    ...
    /**
     * <b>ATTRIBUTE:</b> a switcher indicate whiter load more function is enabled
     */
    ...
    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status release to refresh -> refreshing
     */
    ...
    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status refresh complete -> default {@link #setRefreshing(boolean)} false
     */
    private int mRefreshCompleteToDefaultScrollingDuration = DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION;

    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status default -> refreshing, mainly for auto refresh
    ...
    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status release to loading more -> loading more
     */
    ...
    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status load more complete -> default {@link #setLoadingMore(boolean)} false
     */
    ...
    /**
     * <b>ATTRIBUTE:</b> Scrolling duration status default -> loading more, mainly for auto load more
    ...
    public SwipeToLoadLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                } else if (attr == R.styleable.SwipeToLoadLayout_header_view) {
                    mHeaderViewResId = a.getResourceId(R.styleable.SwipeToLoadLayout_header_view, -1);
                } else if (attr == R.styleable.SwipeToLoadLayout_footer_view) {
                    mFooterViewResId = a.getResourceId(R.styleable.SwipeToLoadLayout_footer_view, -1);
                } else if (attr == R.styleable.SwipeToLoadLayout_target_view) {
                    mTargetViewResId = a.getResourceId(R.styleable.SwipeToLoadLayout_target_view, -1);
                }
            }
        } finally {
            a.recycle();
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mTargetView = findViewById(mTargetViewResId);
            mFooterView = findViewById(mFooterViewResId);
        } else {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_CANCEL:
                printLog("dispatchTouchEvent ACTION_CANCEL");
                break;
            case MotionEvent.ACTION_UP:
                ...
                printLog("dispatchTouchEvent ACTION_UP");
                onActivePointerUp();
                break;
            default:
                break;
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                // 处理手指松开 同 case MotionEvent.ACTION_CANCEL:
            case MotionEvent.ACTION_CANCEL:
     ...
     *
     * @return True for being refreshing enabled
     */
     ...
     *
     * @return True for load more enabled
     */
     ...
     *
     * @return Ture for being refreshing
     */
    ...
    /**
     * set refresh header view, the view must at lease be an implement of {@code SwipeRefreshTrigger}. the view can also
     * implement {@code SwipeTrigger} for more extension functions
     *
     * @param view View
     */
    ...
    /**
     * set load more footer view, the view must at least be an implement of SwipeLoadTrigger the view can also implement
    ...
    /**
     * set the value of {@link #mRefreshTriggerOffset}. Default value is the refresh header view height
     * {@link #mHeaderHeight}
     * <p/>
     * If the offset you set is smaller than {@link #mHeaderHeight} or not set, using {@link #mHeaderHeight} as default
     * value
     *
    ...
    /**
     * set the value of {@link #mLoadMoreTriggerOffset}. Default value is the load more footer view height
     * {@link #mFooterHeight}
     * <p/>
     * If the offset you set is smaller than {@link #mFooterHeight} or not set, using {@link #mFooterHeight} as default
     * value
     *
     * @param offset The offset
     */
     ...
     * If the offset you set is 0(default value) or smaller than {@link #mRefreshTriggerOffset} there no final offset
     *
     * @param offset The offset
     */
     ...
     * If the offset you set is 0(default value) or smaller than {@link #mLoadMoreTriggerOffset}, there no final offset
     *
     * @param offset The offset
     */
     ...
     *
     * @param refreshing true for refreshing
     */
     ...
     *
     * @param loadingMore true for londing more
     */
    ...
    /**
     * copy from {@link androidx.swiperefreshlayout.widget.SwipeRefreshLayout#canChildScrollUp()}
     *
    ...
    /**
     * Whether it is possible for the child view of this layout to scroll down. Override this if the child view is a
     * custom view.
     *
     * @return True for can 
     */
    ...
    void autoScroll(final float yScrolled) {
    ...
    /**
     * Process the scrolling(auto or physical) and append the diff values to mTargetOffset I think it's the most busy
     * and core method. :) a ha ha ha ha...
     *
     * @param yScrolled Scrolled by y
     */
    ...
    void scrollLoadingMoreToDefault() {
    ...
    final RefreshCallback mRefreshCallback = new RefreshCallback() {
        @Override
    ...
    final LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
     ...
     */
    abstract static class RefreshCallback implements SwipeTrigger, SwipeRefreshTrigger {
    }

    /**
     * load more event callback
     */
    abstract static class LoadMoreCallback implements SwipeTrigger, SwipeLoadMoreTrigger {
    }
    ...
    private class AutoScroller implements Runnable {
        ...
        /**
         * The param yScrolled here isn't final pos of y. It's just like the yScrolled param in the
         * {@link #updateScroll(float yScrolled)}
         *
         * @param yScrolled y
         * @param duration The duration
         */
        void autoScroll(int yScrolled, int duration) {
            removeCallbacks(this);
            mmLastY = 0;
            if (!mScroller.isFinished()) {
                mScroller.forceFinished(true);
    ...
    private static final class STATUS {
        ...
        static boolean isRefreshing(final int status) {
            return status == STATUS.STATUS_REFRESHING;
        }

        static boolean isLoadingMore(final int status) {
            return status == STATUS.STATUS_LOADING_MORE;
        }

        static boolean isReleaseToRefresh(final int status) {
            return status == STATUS.STATUS_RELEASE_TO_REFRESH;
        }

        static boolean isReleaseToLoadMore(final int status) {
            return status == STATUS.STATUS_RELEASE_TO_LOAD_MORE;
        }

        static boolean isSwipingToRefresh(final int status) {
            return status == STATUS.STATUS_SWIPING_TO_REFRESH;
        }

        static boolean isSwipingToLoadMore(final int status) {
            return status == STATUS.STATUS_SWIPING_TO_LOAD_MORE;
        }

        static boolean isRefreshStatus(final int status) {
        ...
        static boolean isStatusDefault(final int status) {
            return status == STATUS.STATUS_DEFAULT;
        }
